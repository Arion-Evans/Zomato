...
* log(Pr(Average)/Pr(Poor)) = 3.999 - 1.211(Has.Online.Delivery) - 0.007(Votes)
* log(Pr(Good)/Pr(Poor)) = 2.215 - 1.151(Has.Online.Delivery) - 0.006(Votes)
* log(Pr(Very Good)/Pr(Poor)) = 1.296 - 1.870(Has.Online.Delivery) - 0.007(Votes)
* log(Pr(Excellent)/Pr(Poor)) = -0.036 - 2.8131(Has.Online.Delivery) - 0.008(Votes)
...
Here, it can be seen restaurants with online delivery are .298 as likely than restaurants without online delivery to rated average rather than poor. Furthermore, this trend is shown across all the levels, suggesting that generally, a restaurant with online delivery is more likely to be rated poor than any other level. For the Votes variable, a one unit increase in votes is shown to increase the odds of a restaurant being rated excellent as opposed to poor by 1.008. In fact, an increase in votes essentially translates into a restaurant being rating anything other poor across all levels, except for the Average level.

\newpage

# Model Prediction

In order to gain some more insight into how the predictor variables effect the response level probabilities, a new dataset was constructed and fed into the model. The results are plotted below:
...
The full dataset was fed into the prediction model to obtain prediction response for the `Rating.text` variable. These results are compared to the actual observations in the plot below:

```{r, echo=FALSE}
#generating predictions over dataset
pred<-data.table(predict(fit.bic.nom,newdata = zom.r, type = "class"))
pred[, `:=` (Count = .N), by=V1]
...
```{r}
ci.table<-MultinomCI(pred$Count,.95,"wald")
ci.table#not sure if right...
```
